...
def add_lines_to_final_output(final_output, old_file, updated_file)
  ...
  final_emails = final_file_data['Email']  # Assuming 'Email' is the column header for email addresses
  added_rows = []

  # Iterate through the rows in the old file
  old_file_data.each do |row|
    email = row['Email']  # Assuming 'Email' is the column header for email addresses
  ...
  final_file_data.concat(added_rows)
